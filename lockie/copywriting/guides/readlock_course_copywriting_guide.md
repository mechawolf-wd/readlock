...
**Variant H (first-person testimony) uses "I".** This is the one variant where a first-person singular pronoun drives the narration. The protagonist speaks as themselves. "I" replaces the third-person narrator for the duration of Phase 2. The rest of the package (intro, questions, reveal, outro) stays in the normal narrator voice.
...
**Stories can include a line of dialogue.** In variants A (reader-immersion), B (single person), C (two-people contrast), D (diverse multiple), F (historical arc), and H (first-person testimony), a `@text` swipe in a story can include a short line of someone speaking. Use it when the spoken line brings the scene closer to the reader. One line per swipe. Two at most, and only sometimes. In Variant H the protagonist can quote others they encountered ("My manager said, 'Just pick whichever looks right.'"). (Variants E and G are excluded because E builds from mechanism and G from hypothetical logic, so neither has a real character whose voice would help.)
